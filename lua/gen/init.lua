    ...
    local function substitute_placeholders(input)
        ...
        text = string.gsub(text, '%$register_([%w*+:/"])', function(r_name)
            ...
            if not register or register:match("^%s*$") then
                error("Prompt uses $register_" .. r_name .. " but register " .. r_name .. " is empty")
            end
            return register
        end)
...
M.run_command = function(cmd, opts)
    ...
    globals.job_id = vim.fn.jobstart(cmd, {
        on_stdout = function(_, data, _)
            ...
            if opts.debug then
                vim.print("[Gen] stdout data:", data)
            end
            ...
            for _, line in ipairs(lines) do
                Process_response(line, opts.json_response, opts.debug)
            end
            if partial_data:sub(-1) == "}" then
                Process_response(partial_data, opts.json_response, opts.debug)
                partial_data = ""
            end
        end,
        on_stderr = function(_, data, _)
            if opts.debug then
                ...
                vim.print("[Gen] stderr data:", data)
...
function Process_response(str, json_response, debug)
    ...
    if json_response then
        ...
        if success then
            ...
            vim.fn.jobstop(globals.job_id)
            if debug then
                vim.print("[Gen] JSON decode error:", str)
            end
            return
        end
    else
    ...
    write_to_buffer(lines)
    if debug then
        vim.print("[Gen] processed text:", text)
    end
end
